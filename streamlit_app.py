...
if uploaded_file_json is not None:
    @st.cache_data
    def load_json(uploaded_file_json):
        return json.load(uploaded_file_json)

    json_data = load_json(uploaded_file_json)
    ...
    if uploaded_file is not None and table_id_input and ingest_button:
        ...
        try:
            ...
            # Verify columns before uploading
            required_columns = ['bu', 'campaign_name', 'group_name', 'subgroup_name', 'target', 'create_date', 
                                'start_campaign', 'end_campaign', 'send_sms', 'send_date_sms', 'send_edm', 
                                'send_date_edm', 'send_line', 'send_date_line', 'send_the1app', 'send_date_the1app', 
                                'send_colapp', 'send_date_colapp', 'send_martech', 'send_date_martech', 'send_facebook', 
                                'send_date_facebook', 'send_call', 'send_date_call', 'requester', 'data_owner', 
                                'member_number']
            missing_columns = [col for col in required_columns if col not in data.columns]
            if missing_columns:
                st.error(f"The following required columns are missing from the CSV file: {', '.join(missing_columns)}")
            else:
